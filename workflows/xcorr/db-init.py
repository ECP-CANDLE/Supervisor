...
def insert_feature_names():
    """Copy features from the header of this datafile into the features
    table."""
    global THIS
    ...
    for name in feature_names:
        if name == "":
            continue
        name = name.strip()
        DB.insert(table="feature_names", names=["name"], values=[q(name)])
...
def insert_study_names():
    ...
    for study in studies:
        DB.insert(table="study_names", names=["name"], values=[q(study)])
