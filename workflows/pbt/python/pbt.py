...
class PBTClient:
    """Client of the PBTMetaDataStore, used to request locks, and put and get
    data from a PBTMetaDataStore."""

    def __init__(self, comm, dest, outdir):
        """Initializes the PBT client with a communicator and the destination
        rank of the PBTMetaDataStore.
    ...
    def release_write_lock(self, for_rank):
        """Releases the write lock for the specified rank that has been
        acquired by the put_data call.
    ...
    def get_data(self, score, lock_weights=True):
        """Gets the metadata for a better performing model, assuming there is
        one.
    ...
    def done(self):
        """Notifies the PBTMetaDataStore that model associated with this
        PBTClient is finished.

        No more PBTClient calls should be made after this method is
        called.
        """
...
class DataSpacesPBTClient(PBTClient):
    ...
    def put(self, data, model):
        ...
        self.put_data(data)
        self.lib.pbt_ds_put_weights(self.rank, weights, weights_size,
                                    self.mpi_comm_self)
        self.release_write_lock(self.rank)
...
class PBTMetaDataStore:
    ...
    def run(self):
        ...
        self.logs.append("PBT Start: {}".format(
        ...
        t = time.localtime()
        self.logs.append("PBT End: {}".format(
            time.strftime("%Y-%m-%d %H:%M:%S", t)))
...
class PBTWorker:
    """PBTCallback uses classes that implement this API to determine when a
    model is ready to exploit and explore, to retrieve metadata and
    hyperparameters from the model to put in the shared PBTMetaDataStore, and
    to perform the model specific exploit and explore update."""
    ...
    def pack_data(self, pbt_client, model, metrics):
        ...
        to be passed to the PBTMetaDataStore. A typical implementation will
        select one of the metrics (e.g. 'val_loss') from the keras provided
        metrics and set that as the 'score' used to determine model peformance.
    ...
    def update(self, epoch, pbt_client, model, data):
        """Updates the specified model by performing an exploit / explore using
        the data in data. NOTE that the PBTCallback will load the new weights
        into the model. That should NOT be done here.
...
class PBTCallback(keras.callbacks.Callback):
    ...
    def on_epoch_begin(self, epoch, logs):
        ...
        t = time.localtime()
        self.client.log("Client {} Epoch {} Start: {}".format(
            self.client.rank, epoch, time.strftime("%Y-%m-%d %H:%M:%S", t)))
    ...
    def on_epoch_end(self, epoch, logs):
        ...
        if self.pbt_worker.ready(self.client, self.model, epoch):
            ...
            if len(result):
                print("{},{} is ready - updating".format(
                    epoch, self.client.rank))
                rank_to_read = result["rank"]
    ...
    def on_train_end(self, logs={}):
        t = time.localtime()
        self.client.log("Client {} End: {}".format(
            self.client.rank, time.strftime("%Y-%m-%d %H:%M:%S", t)))
        self.client.done()
