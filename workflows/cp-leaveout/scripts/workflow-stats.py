...
parser.add_argument("--token", default=None, help="User-readable naming token")
...
class Statter:
    """Compute states for some quantity (epochs_actual, stops, val_loss) by
    stage."""
...
def do_percentiles():
    for percentile in [0.99, 0.75, 0.50, 0.25, 0.10]:
        ...
        filename = "percentile-%s-%0.2f.data" % (args.token, percentile)
