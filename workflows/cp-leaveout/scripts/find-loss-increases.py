...
for node_id in data.keys():
    ...
    if current.val_loss > node_loss_worst.loss:
        node_worst = current
    if current.val_loss < node_loss_best.loss:
        node_best = current
    total += 1
...
for node_id in data.keys():
    ...
    if current.val_loss > node_vl_worst.val_loss:
        node_worst = current
    if current.val_loss < node_vl_best.val_loss:
        node_best = current
    total += 1
...
def report_top_loss_deltas():
    ...
    for node in increases_loss:
        ...
        print("%2i %-12s %0.6f %0.6f %0.6f %0.6f" %
              (index, node.id, node.loss, parent.loss, node.get_loss_delta(),
               ratio))
        ratios.append(ratio)
        index += 1
    ratios.sort()
