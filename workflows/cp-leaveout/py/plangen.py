...
class SubsetGenerator(ABC):
    ...
    def partition(self, base, size=None, count=None, name="-unspecified-"):
        ...
        Partition the 'base', a list of elements, using the abstract
        arguments 'size' and 'count' to tailor the implementation's
        algorithm. 'name' is used in error reporting and is optional.
        """
...
class LeaveoutSubsetGenerator(SubsetGenerator):
    ...
    All SubsetGenerator subclasses are required to implement
    partition(), plan_init() and plan_term() functions.
    """
...
PlanstatRow = namedtuple(
    "PlanstatRow",
    [
        "rowid", "plan_name", "create_date", "feature_sets", "partitions",
        "nbr_subplans"
    ],
)
...
def plan_prep(db_path, plan_path, run_type=RunType.RUN_ALL):
    ...
    if run_type == RunType.RUN_ALL and rowid > 0:
        print("Error: RUN_ALL specified but plan: %s has already been defined" %
              plan_key)
        error = True
    ...
    elif (rowid > 0 and create_date != create_date
    ...
    if not error and rowid < 0:
        ...
        stmt = _insert_planstat_plan.format(plan_key, create_date, feature_sets,
                                            partitions, nbr_subplans)
...
def start_subplan(db_path,
    ...
    if not skip:
        ...
        stmt = _insupd_scheduled_runhist.format(plan_id, subplan_id,
...
def stop_subplan(db_path, plan_id=None, subplan_id=None, comp_info_dict={}):
    ...
    if row:  # expected, caller error if already marked COMPLETED
        runhist_rec = RunhistRow._make(row)
        if runhist_rec.status != RunStat.COMPLETE.name:
            start_time = datetime.strptime(runhist_rec.start_time,
                                           ISO_TIMESTAMP_ENCODE)
...
def build_plan_tree(args,
    ...
    for i in range(len(args.fs_names)):
        ...
        partitions = args.generator.partition(feature_set_content[i],
                                              count=count)
...
def main():
    ...
    steps = build_plan_tree(
        ...
        parent_plan_id=
        root_name,  # name of root plan, subplan names created from this stem
        data_pfx=
        data_fname_pfx,  # DATA file prefix, building block for feature name files
        plan_pfx=
        plan_fname_pfx,  # PLAN file prefix, building block for plan name files
    )
...
def test1(plan_path, db_path):
    ...
    print("Demonstrate that root '%s' predecessor is not defined: %s" %
    ...
    for i in it.count(start=1):
        ...
        print("%-16s is a successor of %-16s - all successors: %s" %
        ...
        if i < 3:
            for pf in [False, True]:
                ...
                if (
                        False
                ):  # very verbose, use only as needed! ???????????????????????????????????????????????????????
                    print("\nsubplan original:", select_one, "parent features:",
                          pf)
        ...
        row = get_subplan_runhist(db_path,
                                  plan_id=plan_id,
                                  subplan_id=select_one)
        # print(row)

        # post subplan termination
        completion_status = dict(mse=1.1,
                                 mae=2.2,
                                 r_square=0.555,
                                 misc="no such column",
                                 data=123)
