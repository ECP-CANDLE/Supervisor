...
class Config:
    ...
    INT_KEYS = [
        "PROCS", "PPN", "BENCHMARK_TIMEOUT", "SH_TIMEOUT", "IGNORE_ERRORS"
    ]
        ...
        if "stage_cfgs" in self.cfg:
            for stage_cfg in self.cfg["stage_cfgs"]:
                if not "stage" in stage_cfg:
                    return (
                        False,
                        "A stage_cfg map is missing required 'stage' property",
                    )
                for k in stage_cfg:
                    if k not in Config.STAGE_CFG_KEYS:
                        return (
                            False,
                            "Unknow stage configuration property {}".format(k),
                        )
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=
        "Runs the workflow with actual job submission, displaying each job's configuration",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="the stage to begin the workflow with",
    )
    parser.add_argument(
        ...
        default="",
        help=
        "the directory containing the parent model runs for the initial stage, if initial_stage > 1",
    )
...
def run_upfs(upfs, cfg):
    ...
    for i, upf in enumerate(upfs):
        ...
        print("\n########### JOB {} - Stage {} - {} - {} ##############".format(
            stage - cfg.first_stage + 1, stage, exp_id, job_id))
        print("Running: {} {}".format(cfg.submit_script, " ".join(args)))
        print(outs)
...
def run(args):
    ...
    if cfg.first_stage > total_stages:
        print(
            "First stage must be less than or equal to total number of stages")
        sys.exit()

    if cfg.first_stage > 1 and ("first_stage_parent_directory" not in cfg.cfg or
                                cfg.cfg["first_stage_parent_directory"] == ""):
        print(
            "Missing required 'first_stage_parent_directory' argument, when first_stage > 1"
        )
        sys.exit()
    ...
    print(
        ...
        .format(
            ...
            cfg.plan,
            cfg.submit_script,
            cfg.stage_cfg_script,
            cfg.upf_directory,
        ))
    for i, c in enumerate(runs_per_stage):
        stage = cfg.first_stage + i
        scfg = cfg.stage_cfgs[stage]
        print("\tStage: {}, UPF: {}, Model Runs: {}, PROCS: {}, PPN: {}".format(
            stage, os.path.basename(upfs[i]), c, scfg["PROCS"], scfg["PPN"]))
