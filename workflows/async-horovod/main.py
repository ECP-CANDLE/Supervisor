...
def parse_args():
    ...
    parser.add_argument("parallelism", help="Nodes per Keras run")
    parser.add_argument("points_init", help="Number of initial sample points")
    parser.add_argument("points_max", help="Number of total sample points")
    parser.add_argument("cfg_file", help="The cfg file (see README)")
...
def read_cfg(cfg):
    ...
    defaults = {"launch_delay": 0, "log_level": "INFO"}
...
def setup_run(output):
    ...
    try:
        ...
        fail("could not make output directory: " + output + "\n" + str(e))
    return output
...
def read_val_loss(output, task):
    ...
    try:
        ...
        fail("Could not open result file: " + filename + "\n" + str(e))
    try:
        number = float(result)
    except Exception as e:
        fail('Invalid number "' + result + '" in result file: ' + filename +
             "\n" + str(e))
