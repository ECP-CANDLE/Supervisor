...
# run using source("test/emews_mlrMBO3_run.R")
...
OUT_put <- function(x) {
  if (!last.time){
    if (x == "DONE"){
      ...
      ## Handshake protocol
      for.In_get <<- "max.budget = 30, max.iterations = 2, design.size=3, propose.points=3, param.set.file='../data/parameter_set3.R'"
    }
    else {
