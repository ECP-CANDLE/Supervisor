...
def initialize_parameters():
    ...
    mymodel_common = candle.Benchmark(
        file_path,
...
def run(gParameters):
    ...
    def focal_loss(labels, logits, gamma=0, alpha=1.0):
        ...
        weight = tf.multiply(onehot_labels,
                             tf.pow(tf.subtract(1.0, model_out), gamma))
    ...
    def dice_coef(y_true, y_pred):
        ...
        dc = K.mean((2.0 * intersection + smooth) / (union + smooth), axis=0)
        return dc
    ...
    def get_unet(
        ...
        for i in range(n_layers - 1):
            ...
            up = concatenate(
                [
                    Conv2DTranspose(filter_size, (2, 2),
                                    strides=(2, 2),
                                    padding="same")(conv_layers[-1]),
                    conv_layers[n_layers - i - 2],
                ],
                axis=3,
            )
            conv = Conv2D(filter_size,
        ...
        last_conv = Conv2D(1, (1, 1),
                           activation=last_activation)(conv_layers[-1])
    ...
    def preprocess_images(images):
        ...
        if imgs_train.ndim != 3:
            raise Exception(
                "Error: The number of dimensions for images should equal 3, after squeezing the shape is:{0}"
    ...
    def preprocess_masks(masks, normalize_mask=False):
        ...
        if imgs_mask_train.ndim != 3:
            raise Exception(
                "Error: The number of dimensions for masks should equal 3, after squeezing the shape is:{0}"
                .format(np.shape(masks)))
    ...
    def predict(model, weights, images):
        ...
        np.save("mask_predictions.npy",
                np.squeeze(np.round(imgs_mask_test).astype("uint8")))
    ...
    if not gParameters["predict"]:
        ...
        history_callback = evaluate_params(
            images,
            labels,
            batch_size,
            epochs,
            ...
            batch_norm,
            learning_rate,
        )  # note that history_callback is what's returned by model.fit()
