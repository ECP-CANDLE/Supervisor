...
3. Any number of sequences `K=V`, which sets environment variable `K` to `V` at that point.  Note that you must reset the interpreter with `-` for this to have an effect in Python.
